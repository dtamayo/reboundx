...
double my_density = (3000.0*au_conv*au_conv*au_conv)/msun_conv;
double my_c = (2.998e8*yr_conv)/au_conv;
double my_lstar = (3.828e26*yr_conv*yr_conv*yr_conv)/(msun_conv*au_conv*au_conv);
    
rebx_set_param_double(rebx, &sim->particles[1].ap, "my_body_density", my_density);
rebx_set_param_int(rebx, &sim->particles[1].ap, "direction_flag", 1);
rebx_set_param_double(rebx, &yark->ap, "my_lstar", my_lstar);
rebx_set_param_double(rebx, &yark->ap, "my_c", my_c);
