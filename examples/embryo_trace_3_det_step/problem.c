...
char TITLE_FAMTREE[100] = "famtree_5454";
...
void heartbeat(struct reb_simulation* sim){
    if (reb_simulation_output_check(sim, 1)){
        FILE *fp = fopen("heartbeat_5454.txt", "a");  // append mode
        if (fp == NULL){
...
int main(int argc, char* argv[]){
    char* filename = "/home/tajer.1/reboundx/examples/embryo_TRACE_Jul22/embryo_trace_3";
    struct reb_simulationarchive* sa = reb_simulationarchive_create_from_file(filename);
    struct reb_simulation* r = reb_simulation_create_from_simulationarchive(sa,5454);
    ...
    reb_simulation_save_to_file_step(r, "archive_timestep_5454.bin", 1);
    //reb_simulation_save_to_file_interval(r, "archive_timestep_1.bin", 6./356.);

    reb_simulation_integrate(r, r->t+10);
