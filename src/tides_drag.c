 ...
 * Based on                `Schroder et al. 2008 <***>`_.
...
static void rebx_calculate_tides_drag(struct rebx_extras* const rebx, struct reb_simulation* const sim, const double c, const int source_index){
...
void rebx_tides_drag(struct reb_simulation* const sim, struct rebx_force* const tides_drag, struct reb_particle* const particles, const int N){
    //NEXT
}
...
double rebx_tides_drag_lambda(const double alpha){
    return 0.019*pow(alpha, (4/3));
}

double rebx_tides_drag_t_f(const double primary_mass, const double primary_radius, const double source_luminosity){
    const double base = primary_mass*pow(primary_radius, 2)/source_luminosity;
    return pow(base, (1/3));
}
...
double rebx_tides_drag_omega(const double secondary_period){
    return 2.*M_PI/secondary_period;
}
...
double rebx_tides_drag_Gamma(const double lambda, const double t_f, const double q, const double primary_mass, const double primary_radius, const double orbital_radius, const double Omega, const double omega){
    const double radius_ratio = primary_radius / orbital_radius;
    return 6.*(lambda / t_f)*pow(q, 2)*primary_mass*pow(primary_radius, 2)*pow(radius_ratio, 6)*(Omega - omega);
}