...
const char* rebx_version_str = "3.7.1";         // **VERSIONLINE** This line gets updated automatically. Do not edit manually.
...
struct rebx_force* rebx_load_force(struct rebx_extras* const rebx, const char* name){
    ...
    else if (strcmp(name, "triaxial_torque_force") == 0){
        force->update_accelerations = rebx_triaxial_torque_force;
        force->force_type = REBX_FORCE_POS;
    }
    else{
        char str[300];
