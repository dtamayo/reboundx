...
resonances = OrderedDict([(('b','c'),(8,5)),(('c','d'),(5,3)),(('d','e'),(3,2)),(('e','f'),(3,2)),(('f','g'),(4,3)),(('g','h'),(3,2))])
Pd = 4.0496/365.25
...
minmass = 1.e10 # arbitrary large number
for p in ps[1:]:
    if p.m < minmass:
        minmass = p.m
...
mref = ps[1].m
for p in ps[1:]:
    p.params["tides_synchronous_tau_e"] = -tauetides*(p.m/mref)*(p.a/aref)**6.5

rebx.integrator="rk4"
damping.force_as_operator = 1
damping.operator_order = 1

filename='data/IC{0}K{1:.4e}mag{2:.4e}taue{3:.2e}'.format(simID, K, mag,tauetides)
