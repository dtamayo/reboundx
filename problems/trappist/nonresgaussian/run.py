...
path='data/' # path to nonres simulation archives
...
eccdist = {'b':[0,0.081], 'c':[0,0.083], 'd':[0,0.07], 'e':[0,0.085], 'f':[0,0.063], 'g':[0,0.061], 'h':[0,0.07]} # 2sigma from Gillon17 (value for h assumed)
for ecc in eccdist.values(): # convert to 1-sigma
    ecc[1] /= 2.
...
sim.G = 4*np.pi**2 # units of AU, yrs and solar masses
ps = sim.particles
...
sim.initSimulationArchive(path+filename, interval=1000.)
try:
    sim.integrate(tmax)
except:
    print(sim.t)
